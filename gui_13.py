...
def modulacion_isb(audio_L, audio_R):
    ...
    carrier_cos = np.cos(2*np.pi*FC*t)
    carrier_sin = np.sin(2*np.pi*FC*t)
    analytic_L = np.imag(hilbert(audio_L))
    analytic_R = np.imag(hilbert(audio_R))
    isb_usb = np.real(audio_L * carrier_cos - analytic_L * carrier_sin)
    isb_lsb = np.real(audio_R * carrier_cos + analytic_R * carrier_sin)
...
def grabar_en_hilo():
    ...
    try:
        ...
        estado_var.set("🎚 Guardando ISB - L")
        root.update()
        write(archivo_L_ISB, fs, audio_estereo[:, 0])

        estado_var.set("🎚 Guardando ISB - R ")
        root.update()
