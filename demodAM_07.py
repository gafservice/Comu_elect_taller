...
def main():
    ...
    print("🔁 Sistema activo. Esperando tono de 2000 Hz...")
    ...
    while True:
        ...
        def callback_inicial(indata, frames, time_info, status):
            ...
            bloque = indata[:, 0]
            detectado, f, S = detectar_tono(bloque, 2000, fs, margen=30, umbral=umbral_inicio)
        ...
        print("⏺️ Grabando mensaje hasta detectar tono de fin (3000 Hz)...")

        def callback_mensaje(indata, frames, time_info, status):
            nonlocal mensaje
            bloque = indata[:, 0]
            detectado_fin, _, _ = detectar_tono(bloque, 3000, fs, margen=30, umbral=umbral_fin)
