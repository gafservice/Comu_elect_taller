    ...
    def reproducir_audio_especifico(nombre_archivo):
        try:
            fs, data = wavfile.read(nombre_archivo)
            sd.play(data, fs)
            sd.wait()
            log_estado(f"Reproduciendo {nombre_archivo}")
        except Exception as e:
            log_estado(f"Error al reproducir {nombre_archivo}: {str(e)}")

    tk.Button(root, text="Reproducir audio_mono.wav", command=lambda: reproducir_audio_especifico("audio_mono.wav"),
              bg="blue", fg="white", font=("Arial", 9, "bold")).place(x=40, y=650, width=200, height=30)

    tk.Button(root, text="Reproducir audio_L_ISB.wav", command=lambda: reproducir_audio_especifico("audio_L_ISB.wav"),
              bg="blue", fg="white", font=("Arial", 9, "bold")).place(x=260, y=650, width=200, height=30)

    tk.Button(root, text="Reproducir audio_R_ISB.wav", command=lambda: reproducir_audio_especifico("audio_R_ISB.wav"),
              bg="blue", fg="white", font=("Arial", 9, "bold")).place(x=40, y=690, width=200, height=30)
import numpy as np
