...
def graficar_espectro_isb(lsb, usb, fs):
    ...
    S_lsb_db = 20 * np.log10(S_lsb + 1e-12)
    S_usb_db = 20 * np.log10(S_usb + 1e-12)
    plt.plot(f[:N//2], S_lsb_db[:N//2], label="LSB (audio_L_ISB)", linestyle='--')
    plt.plot(f[:N//2], S_usb_db[:N//2], label="USB (audio_R_ISB)", linestyle='-')
    plt.title("Espectro de ISB (LSB y USB en dB)")
    plt.xlabel("Frecuencia [Hz]")
    plt.ylabel("Magnitud [dB]")
    plt.legend()
    plt.grid(True)
